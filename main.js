...
const DATA_GAMES = [
  ...
  },
  {
    id: "genshin",
    name: "Genshin Impact",
    image: "https://i.ibb.co/SJp0jsk/genshin.jpg",
    products: [
      { name: "60 Genesis Crystals", price: "Rp 15.000" },
      { name: "330 Genesis Crystals", price: "Rp 75.000" },
      { name: "980 Genesis Crystals", price: "Rp 225.000" },
    ],
  },
];
...
function renderSlider() {
  ...
  SLIDER_IMAGES.forEach((url) => {
    ...
    div.className = "slide";
    div.innerHTML = `<img src="${url}" alt="Promo">`;
...
function prevSlide() {
  currentSlide =
    (currentSlide - 1 + SLIDER_IMAGES.length) % SLIDER_IMAGES.length;
  updateSlide();
}
...
function renderGames() {
  ...
  DATA_GAMES.forEach((g) => {
    ...
    card.innerHTML = `
      <img src="${g.image}" alt="${g.name}" width="60" height="60">
      <h3>${g.name}</h3>
      <small>Klik untuk top up</small>`;
  ...
  section.innerHTML = `
    <div class="panel">
      ...
      <div class="order-wrap">
        <div>
          <div class="field">
            <label>ID Player / Email</label>
            <input id="userId" placeholder="Masukkan ID Game">
          </div>
          <div class="field">
            <label>Server ID (opsional)</label>
            <input id="serverId" placeholder="Masukkan Server (opsional)">
          </div>
          ...
          <div class="field">
            ...
            <div class="products-grid" id="productList">
              ${item.products
                .map(
                  (p, i) => `
                <div class="product-card" onclick="selectProduct(this, ${i}, '${item.id}')">
          ...
          <div class="field">
            ...
            <div class="payment-grid" id="paymentList">
              ${DATA_PAYMENTS.map(
                (p, i) => `
                <div class="payment-card" onclick="selectPayment(this, ${i})">${p.name}</div>`
              ).join("")}
            </div>
          </div>
...
function selectProduct(el, i, id) {
  document
    .querySelectorAll("#productList .product-card")
    .forEach((c) => c.classList.remove("active"));
  el.classList.add("active");
  const item = DATA_GAMES.find((x) => x.id === id);
...
function selectPayment(el, i) {
  document
    .querySelectorAll("#paymentList .payment-card")
    .forEach((c) => c.classList.remove("active"));
  el.classList.add("active");
  selectedPayment = DATA_PAYMENTS[i];
  updatePreview();
}
...
function updatePreview() {
  ...
  preview.textContent = text || "Belum ada produk dipilih.";
}
...
function orderNow(gameName) {
  ...
  const msg = `Halo kak, saya ingin top up:%0A%0A🎮 Game: ${encodeURIComponent(
    gameName
  )}%0A🆔 ID: ${encodeURIComponent(id)}${
    server ? `%0A🌐 Server: ${encodeURIComponent(server)}` : ""
  }%0A📦 Produk: ${encodeURIComponent(
    selectedProduct.name
  )}%0A💰 Harga: ${encodeURIComponent(
    selectedProduct.price
  )}%0A💳 Metode: ${encodeURIComponent(selectedPayment.name)}`;
