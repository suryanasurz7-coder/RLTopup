...
function renderSlider() {
  ...
  SLIDER_IMAGES.forEach((url) => {
    ...
    d.className = "slide";
    d.innerHTML = `<img src="${url}" alt="Promo">`;
    s.appendChild(d);
  });
  updateSlide();
  startAutoSlide();
          ...
          <div class="field"><label>Pilih Produk</label>
            <div class="products-grid" id="productList">
              ${item.products
                .map(
                  (p, i) =>
                    `<div class="product-card" onclick="selectProduct(this,${i},'${item.id}')"><div class='title'>${p.name}</div><div class='price'>${p.price}</div></div>`
                )
                .join("")}
            </div>
          </div>
...
function updatePreview() {
  const id = document.getElementById("userId")?.value.trim() || "",
    server = document.getElementById("serverId")?.value.trim() || "";
...
function orderNow(itemName) {
  ...
  const msg = `Halo kak, saya ingin top up:%0A%0AGame: ${encodeURIComponent(
    itemName
  )}%0AID: ${encodeURIComponent(id)}${
    server ? `%0AServer: ${encodeURIComponent(server)}` : ""
  }%0AProduk: ${encodeURIComponent(
    selectedProduct.name
  )}%0AHarga: ${encodeURIComponent(
    selectedProduct.price
  )}%0AMetode: ${encodeURIComponent(selectedPayment.name)}`;
